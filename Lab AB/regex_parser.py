...
class ShuntingYard:
    ...
    def validate_operators(self, regex):
        for i, char in enumerate(regex):
            if char in self.unary_operators and (i == 0 or regex[i - 1] in self.all_operators + ['(']):
            ...
            if char in self.binary_operators:
                ...
                if regex[i - 1] in self.all_operators + ['('] or regex[i + 1] in self.all_operators + [')']:
                    raise ValueError(f"Operador binario '{char}' usado incorrectamente en la posición {i}.")

        for i in range(len(regex) - 1):
            if regex[i] in self.unary_operators and regex[i + 1] in self.unary_operators:
                raise ValueError(f"Secuencia de operadores de clausura '{regex[i]}{regex[i + 1]}' no es válida en la posición {i}.")
